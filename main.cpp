#include <pcap.h> // libpcap 라이브러리 헤더: 네트워크 패킷 캡처 및 전송 기능을 제공합니다.
#include <netinet/ip.h> // IP 헤더 구조체 (struct ip) 정의를 포함합니다.
#include <netinet/tcp.h> // TCP 헤더 구조체 (struct tcphdr) 정의를 포함합니다.
#include <netinet/ether.h> // 이더넷 헤더 구조체 (struct ether_header) 정의를 포함합니다.
#include <arpa/inet.h> // 네트워크 주소 변환 함수 (예: ntohs, htonl, inet_ntoa)를 포함합니다.
#include <unistd.h> // POSIX 운영체제 API (예: close 함수)를 포함합니다.
#include <cstring> // 문자열 및 메모리 조작 함수 (예: memcpy, memset)를 포함합니다.
#include <iostream> // 표준 입출력 스트림 (예: cout, cerr)을 사용하기 위한 헤더입니다.

using namespace std; // std 네임스페이스를 사용하여 cout, cerr, string 등을 직접 사용할 수 있게 합니다.
...
void usage() { // 프로그램 사용법을 출력하는 함수입니다.
    cout << "Usage: ./tcp-block <interface> <pattern>\n"; // 올바른 실행 인자 형식을 안내합니다.
}

uint16_t checksum(uint16_t *buf, int len) { // IP 및 TCP 헤더 체크섬을 계산하는 함수입니다.
    uint32_t sum = 0; // 체크섬 계산을 위한 32비트 합계 변수입니다.
    while (len > 1) { // 2바이트(16비트) 단위로 버퍼를 순회하며 합산합니다.
...
struct PseudoHeader { // TCP 체크섬 계산에 사용되는 의사(Pseudo) 헤더 구조체입니다.
    uint32_t src; // 소스 IP 주소입니다.
    uint32_t dst; // 목적지 IP 주소입니다.
    uint8_t zero = 0; // 항상 0으로 설정되는 필드입니다.
    uint8_t proto = IPPROTO_TCP; // 프로토콜 타입 (TCP의 경우 6)입니다.
...
void inject_tcp(const ip *ip_src, const tcphdr *tcp_src, const char *data, int data_len, uint8_t flags) {
    // TCP 패킷을 생성하여 주입(inject)하는 함수입니다.
    char buf[BUF_SIZE] = {}; // 패킷 생성을 위한 버퍼를 0으로 초기화합니다.
    ip *iph = (ip *)buf; // 버퍼 시작 부분을 IP 헤더로 캐스팅합니다.
    tcphdr *tcph = (tcphdr *)(buf + sizeof(ip)); // IP 헤더 다음 부분을 TCP 헤더로 캐스팅합니다.
    char *payload = buf + sizeof(ip) + sizeof(tcphdr); // TCP 헤더 다음 부분을 페이로드 시작 부분으로 캐스팅합니다.

    if (data && data_len > 0) // 전송할 데이터가 있고 길이가 0보다 크면
        memcpy(payload, data, data_len); // 페이로드 영역에 데이터를 복사합니다.

    iph->ip_v = 4; // IP 버전 4로 설정합니다.
    iph->ip_hl = 5; // IP 헤더 길이 (5 * 4 = 20 바이트)로 설정합니다.
    iph->ip_ttl = 64; // Time To Live (TTL) 값을 64로 설정합니다.
    iph->ip_p = IPPROTO_TCP; // 프로토콜을 TCP로 설정합니다.
    iph->ip_len = htons(sizeof(ip) + sizeof(tcphdr) + data_len);
    // IP 패킷의 총 길이를 네트워크 바이트 순서로 설정합니다 (IP 헤더 + TCP 헤더 + 데이터).
    iph->ip_off = 0; // 단편화 오프셋을 0으로 설정합니다 (단편화 없음).
    iph->ip_id = htons(54321); // IP 식별자 (ID)를 임의의 값으로 설정합니다.

    // RST 플래그가 설정된 경우 (클라이언트에게 RST를 보낼 때)
    // 소스 IP는 원래 패킷의 목적지 IP, 목적지 IP는 원래 패킷의 소스 IP가 됩니다.
    // 그렇지 않은 경우 (서버에게 FIN/ACK를 보낼 때)
    // 소스 IP는 원래 패킷의 소스 IP, 목적지 IP는 원래 패킷의 목적지 IP가 됩니다.
    ...
    int ip_len = ip_src->ip_hl * 4; // 원래 IP 헤더 길이입니다.
    int tcp_len = tcp_src->th_off * 4; // 원래 TCP 헤더 길이입니다.
    int orig_data_len = ntohs(ip_src->ip_len) - ip_len - tcp_len; // 원래 패킷의 페이로드 길이입니다.
    ...
    // 🔧 수정된 구조체 초기화 방식: TCP 체크섬 계산을 위한 의사 헤더를 설정합니다.
    PseudoHeader pseudo; // 의사 헤더 구조체를 선언합니다.
    pseudo.src = iph->ip_src.s_addr; // 의사 헤더의 소스 IP를 설정합니다.
    pseudo.dst = iph->ip_dst.s_addr; // 의사 헤더의 목적지 IP를 설정합니다.
    pseudo.zero = 0; // 의사 헤더의 zero 필드를 0으로 설정합니다.
    pseudo.proto = IPPROTO_TCP; // 의사 헤더의 프로토콜을 TCP로 설정합니다.
    pseudo.len = htons(sizeof(tcphdr) + data_len); // 의사 헤더의 길이를 TCP 헤더 + 데이터 길이로 설정합니다.

    char pseudo_buf[BUF_SIZE] = {}; // 의사 헤더와 TCP 헤더+데이터를 합칠 버퍼를 0으로 초기화합니다.
    memcpy(pseudo_buf, &pseudo, sizeof(pseudo)); // 의사 헤더를 버퍼에 복사합니다.
    memcpy(pseudo_buf + sizeof(pseudo), tcph, sizeof(tcphdr) + data_len);
    // 의사 헤더 다음 위치에 TCP 헤더와 데이터를 복사합니다.
    tcph->th_sum = checksum((uint16_t *)pseudo_buf, sizeof(pseudo) + sizeof(tcphdr) + data_len);
    // 의사 헤더와 TCP 헤더+데이터를 포함한 전체에 대해 체크섬을 계산하여 TCP 헤더에 설정합니다.

    int sock = socket(AF_INET, SOCK_RAW, IPPROTO_RAW);
    // RAW 소켓을 생성합니다. IPPROTO_RAW는 IP 헤더를 직접 구성함을 의미합니다.
    if (sock < 0) { // 소켓 생성 실패 시 오류 메시지를 출력하고 함수를 종료합니다.
        perror("raw socket"); // errno에 따른 시스템 오류 메시지를 출력합니다.
        return;
    }

    int on = 1; // IP_HDRINCL 옵션을 활성화하기 위한 변수입니다.
    setsockopt(sock, IPPROTO_IP, IP_HDRINCL, &on, sizeof(on));
    // 소켓 옵션을 설정합니다. IP_HDRINCL은 IP 헤더를 직접 포함하여 전송하겠다는 의미입니다.
    // (이 옵션이 없으면 커널이 자동으로 IP 헤더를 생성합니다.)

    sockaddr_in to; // 목적지 주소 정보를 저장할 구조체입니다.
    memset(&to, 0, sizeof(to)); // 구조체를 0으로 초기화합니다.
    to.sin_family = AF_INET; // 주소 체계를 IPv4로 설정합니다.
    to.sin_addr = iph->ip_dst; // 목적지 IP 주소를 설정합니다.

    sendto(sock, buf, sizeof(ip) + sizeof(tcphdr) + data_len, 0, (sockaddr *)&to, sizeof(to));
    // RAW 소켓을 통해 구성된 패킷을 목적지로 전송합니다.
    close(sock); // 소켓을 닫습니다.
}
...
int main(int argc, char *argv[]) { // 프로그램의 메인 함수입니다.
    ...
    // 네트워크 인터페이스(argv[1])를 열어 라이브 패킷 캡처를 시작합니다.
    ...
    // 현재 캡처를 시작한 인터페이스와 감지할 패턴을 출력합니다.

    while (true) { // 무한 루프를 통해 계속해서 패킷을 캡처하고 처리합니다.
        pcap_pkthdr *hdr; // 캡처된 패킷의 헤더 정보를 저장할 포인터입니다.
        const u_char *packet; // 캡처된 패킷 데이터를 가리킬 포인터입니다.
        int ret = pcap_next_ex(handle, &hdr, &packet);
        // 다음 패킷을 캡처합니다. 성공 시 1, 타임아웃 시 0, 오류 시 -1, EOF 시 -2를 반환합니다.
        if (ret <= 0) continue; // 패킷 캡처에 실패하거나 타임아웃된 경우 다음 루프로 넘어갑니다.

        if (contains_pattern(packet, hdr->len, argv[2])) {
            // 캡처된 패킷이 지정된 패턴을 포함하는지 확인합니다.
            const ip *ip_hdr = (ip *)(packet + sizeof(ether_header));
            // 캡처된 패킷에서 IP 헤더를 추출합니다.
            const tcphdr *tcp_hdr = (tcphdr *)((u_char *)ip_hdr + ip_hdr->ip_hl * 4);
            // IP 헤더에서 TCP 헤더를 추출합니다.
            cout << "[!] Match detected. Sending RST and FIN+Redirect." << endl;
            // 패턴이 감지되었음을 알리고 RST 및 FIN+Redirect 패킷을 전송할 것임을 출력합니다.
            inject_tcp(ip_hdr, tcp_hdr, nullptr, 0, TH_RST);
            // 클라이언트에게 RST (Reset) 패킷을 전송하여 현재 TCP 연결을 강제로 종료시킵니다.
            inject_tcp(ip_hdr, tcp_hdr, REDIRECT_MSG.c_str(), REDIRECT_MSG.length(), TH_FIN | TH_ACK);
            // 서버에게 FIN (Finish) 및 ACK (Acknowledgement) 플래그가 설정된 패킷과 함께
            // 리다이렉트 메시지를 전송하여 연결 종료를 알리고 리다이렉트를 유도합니다.
            // (이 부분은 실제 HTTP 리다이렉션이 아닌, 서버 측에 FIN을 보내는 동시에
            // 클라이언트에게 보낼 리다이렉트 메시지를 페이로드에 담아 보내는 시도입니다.
            // 실제 동작은 네트워크 환경 및 브라우저/애플리케이션에 따라 다를 수 있습니다.)
        }
    }
